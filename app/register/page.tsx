    ...
    >
      ...
      <form action={action} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
          <label htmlFor="name" style={{ fontSize: "1rem", color: "var(--text-muted)" }}>
            Nom
          </label>
          <input
            id="name"
          ...
          />
          {state.errors?.name && (
            <span style={errorStyle}>{state.errors.name[0]}</span>
          )}
        </div>
      ...
      <p style={{ marginTop: "1.5rem", textAlign: "center", fontSize: "1rem", color: "var(--text-muted)" }}>
        Déjà un compte ?{" "}
        <Link href="/login" style={{ color: "var(--neon-blue)", textDecoration: "none" }}>
          Se connecter
        </Link>
      </p>
    </div>
